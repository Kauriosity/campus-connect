...
const JoinModalForHome = ({
  isOpen,
  onClose,
  collegeName,
  setIsMeetingScheduled,
}) => {
  ...
  useEffect(() => {
    if (isOpen && collegeName) {
      const savedData = localStorage.getItem(`formData_Home`);
      if (savedData) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (collegeName) {
      localStorage.setItem(`formData_Home`, JSON.stringify(formData));
...
export default JoinModalForHome;
